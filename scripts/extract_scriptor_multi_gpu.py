...
class FidDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return returns[0], self.dataset.classes[target], fid
...
def run(args):
    ...
    val_loader = torch.utils.data.DataLoader(
            FidDataset(datasets.ImageFolder(args.meizi_path, transforms['val'])),
    ...
    if True:
        # 
        gpuDeviceIds = [1]
        device = torch.device('cuda:{}'.format(gpuDeviceIds[0]) if torch.cuda.is_available() else 'cpu')
    ...
    for i, batch in enumerate(loader):
        ...
        with torch.no_grad():
            ...
            batch_input_conv = batch_input_conv.to(device) if True else batch_input_conv.cuda()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input-crop', default='square', choices=['square', 'rect'], help='crop the input or not')
    ...
    parser.add_argument('--embeddingFilePath', default='/home/meizi/short5_125.txt', help='embedding file')
